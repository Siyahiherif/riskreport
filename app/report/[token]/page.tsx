...
export default async function ReportDownloadPage({ params }: { params: { token: string } }) {
    ...
    <div className="flex min-h-screen items-center justify-center bg-slate-50 px-6">
      <div className="max-w-xl rounded-2xl bg-white p-8 shadow ring-1 ring-slate-200">
        ...
        <p className="mt-2 text-sm text-slate-700">
          Token: {params.token} — expires {report.expiresAt.toISOString().slice(0, 16).replace("T", " ")} UTC
        </p>
        {expired ? (
          <p className="mt-4 text-sm text-red-600">This link has expired. Please re-run or purchase a new report.</p>
        ) : (
          <a
            href={`/api/report/${params.token}`}
