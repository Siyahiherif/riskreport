      ...
      <div className="mx-auto max-w-4xl px-6 py-10 space-y-4">
        ...
        <form
          ...
          action={async (formData) => {
            ...
            const finalSlug = slugInput || makeSlug(title);
        ...
        >
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow space-y-3">
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <label className="text-sm text-slate-700">
                Status
                <select name="status" defaultValue={post.status} className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm">
              ...
              <label className="text-sm text-slate-700">
                ...
                <input
                  type="datetime-local"
                  name="publishDate"
                  defaultValue={post.publishDate ? new Date(post.publishDate).toISOString().slice(0, 16) : ""}
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <label className="text-sm text-slate-700">
                Category
                <select name="category" defaultValue={post.category} className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm">
                  {categories.map((c) => (
              ...
              <label className="text-sm text-slate-700">
                ...
                <input
                  ...
                  placeholder="dmarc, tls, executive"
                  className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm"
                />
              </label>
            </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow space-y-3">
            ...
            <input name="seoTitle" defaultValue={post.seoTitle ?? ""} placeholder="SEO title (optional)" className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm" />
            <input name="canonicalUrl" defaultValue={post.canonicalUrl ?? ""} placeholder="Canonical URL (optional)" className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm" />
            <label className="flex items-center gap-2 text-sm text-slate-700">
              <input type="checkbox" name="indexable" defaultChecked={post.indexable} className="h-4 w-4" />
              Allow indexing
            </label>
            <input name="focusKeyword" defaultValue={post.focusKeyword ?? ""} placeholder="Focus keyword (optional)" className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm" />
          </div>
